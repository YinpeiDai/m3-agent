...
class VideoGraph:
    ...
    def refresh_equivalences(self):
        ...
        # Sort by node id so the disjoint-set unions happen in a stable
        # order — otherwise set() iteration is hash-dependent and the
        # resulting character_X numbering changes between runs.
        filtered_equivalence_nodes = sorted(set(filtered_equivalence_nodes))
        ...
        # Pre-touch the lowest-id img nodes so character_0..K naturally
        # claim them (face_0 → character_0, face_1 → character_1, ...).
        # Without this, character_X numbering depends on set/dict
        # iteration order. For SimLife the four avatar-anchored face
        # nodes (graph ids 0..3 = Father/Mother/Son/Daughter Sim) are
        # always inserted first via _seed_avatar_face_nodes, so this
        # gives them the matching low character ids deterministically.
        ordered_node_ids = sorted(self.nodes.keys())
        # First pass: img nodes in numeric order so they claim character ids first.
        for x in ordered_node_ids:
            if self.nodes[x].type != 'img':
                continue
            root = find(x)
            tag = f"face_{x}"
            ...
            character_mappings.setdefault(character, []).append(tag)

        # Second pass: voice nodes in numeric order — same character_X
        # if their root is already claimed by an img union, else a fresh
        # character_X for the voice-only identity.
        for x in ordered_node_ids:
            if self.nodes[x].type != 'voice':
                continue
            root = find(x)
            tag = f"voice_{x}"
            if root not in root_to_character:
